...
#### Resolve a Legacy Star ID (`/api/stars/legacy/{v3_id}`)
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `world_id` | int | 0 | Fictional world ID for the fictional name |

**Response:** the same `StarDetail` shape as `/api/stars/{id}`, plus the legacy id and how
the match was made (`gaia`, `tyc`, `hip`, `hd`, `hr` or `gj`).
...
```json
{
  ...
  "v3_id": 7301,
  "match_method": "gaia",
  "data": { "id": 7323, "gj": "1", "hip": "439", "con": "Scl", "mag": 8.755 }
}
```

**Example:**
```bash
curl "http://localhost:8000/api/stars/legacy/7301"
```

> **This endpoint answers "what did this id used to mean", and nothing more.** It cannot
> tell you whether a given incoming id *is* a legacy id — that is not knowable from the id,
> which is the whole problem. Deciding is the client's job, and the honest answer is to show
> the reader both readings rather than guess. See the `c` parameter under
> [Main Interface](#main-interface-indexphp).

Returns **404** when no v3.3 star maps to that id: it never existed in v3.3, or its star did
not survive to v4 (22 such), or its identifier is shared by two real binary components and
the matcher refused to guess between them.
...
| `c` | int | Catalog version that `select_star` belongs to. Links generated by the app carry `c=4`. |

**Example (deep link):**
```
/index.php?x_c=5&y_c=-3&z_c=0&xy_zoom=15&z_zoom=15&select_star=12345&c=4
```

**The `c` parameter, and why a link without it is ambiguous.** AT-HYG 4 renumbered every
star, and 99.99% of old ids are also valid current ids naming a *different* star, so a bare
`select_star` cannot be attributed to a catalog by inspection. `c=4` marks an id as current.
An id arriving **without** the marker is resolved both ways, and when the two readings
differ the page shows both and lets the reader choose — it never picks silently. Guessing
is not safer in either direction: treating every unmarked id as legacy would break every
link saved since the migration in exactly the same way.

The React app uses the same marker on its `?star=` links.

Worked example: `?select_star=7301` shows an anonymous mag 11.5 star in Cepheus today. It
used to mean GJ 1 — mag 8.755, in Sculptor — now at id 7323.
