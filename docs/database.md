...
2,552,145 rows. Loaded by `db/sql/11_import_athyg_v3_ids.sql` from
...
offset is constant across long runs — the same mapping is **78,733 ranges instead of
2,552,145 rows, and 2 MB instead of 51 MB**. That matters because the file is committed
...
Of the 2,552,145 mapped stars, **only 636 kept the same id** across the migration.
