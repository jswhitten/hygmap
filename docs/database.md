...
### `athyg_v3_ids` - Legacy Star ID Map

Maps **AT-HYG v3.3** star ids onto current ids, so links saved before the AT-HYG 4
migration can be recognised rather than silently resolving to a different star.

| Column | Type | Description |
|--------|------|-------------|
| `v3_id` | INTEGER PRIMARY KEY | Star id in AT-HYG v3.3 |
| `athyg_id` | INTEGER NOT NULL REFERENCES athyg(id) | The star it names today |
| `match_method` | TEXT NOT NULL | Identifier used: `gaia`, `tyc`, `hip`, `hd`, `hr` or `gj` |

2,552,143 rows. Loaded by `db/sql/11_import_athyg_v3_ids.sql` from
`db/data/athyg_v3_ids.csv`, generated by `db/scripts/match_athyg_v3.py`.

**The CSV holds ranges, not one row per id.** Its columns are
`v3_start,v3_end,offset,match_method`, meaning *for every v3 id from `v3_start` to
`v3_end` inclusive, `athyg_id = v3_id + offset`*; the import expands them with
`generate_series`. AT-HYG 4 moved the catalog in blocks rather than shuffling it, so the
offset is constant across long runs — the same mapping is **79,681 ranges instead of
2,552,143 rows, and 2 MB instead of 51 MB**. That matters because the file is committed
deliberately: AT-HYG deleted the v3.3 source from its main branch six days before this
feature was built, so the derived mapping is the durable artifact and needs to stay
comfortably under GitHub's file-size limits.

A range breaks whenever the offset changes, the match method changes, or a v3 id is
missing. The gaps are load-bearing — expanding across one would invent a mapping for a
dead link. The table's `PRIMARY KEY` catches overlapping ranges (they would expand to a
duplicate `v3_id`) and the import rejects inverted ones before expansion, so neither way
of corrupting the file can pass silently.

**Why a table and not a column on `athyg`.** The mapping is **many-to-one**: 5 current
stars are each named by two v3 ids, because AT-HYG 4 merged v3.3 rows. A column could hold
only one of them, silently dropping five links — and `UPDATE ... FROM` with two matching
source rows picks its winner *nondeterministically*, the same class of bug the `ORDER BY`
tiebreaker fixed. A table states the real cardinality: `v3_id` is unique, `athyg_id` is not.

**Ambiguous identifiers are absent by design.** 1,166 Gaia ids and 61 HIP ids each name two
real binary components (see the duplicate-Gaia discussion above), so the matcher refuses
them rather than guessing; those legacy ids resolve to nothing, which is honest. 22 further
v3.3 stars did not survive to v4 at all.

Of the 2,552,143 mapped stars, **only 636 kept the same id** across the migration.
